...
# Define paths
vcf_path = "data/hprc8424.CHM13.merged.norm.vcf.gz"
...
def extract_all_pc1_values(chromosomes):
    ...
    # ADD THE DEBUG PRINTS HERE, right after the existing print statement:
    print(f"About to load population file: {population_file}")
    
    # Load population assignments
    pop_assignments = load_populations(population_file)
    print(f"Population loading completed. Result: {pop_assignments is not None}")
...
if __name__ == "__main__":
    chromosomes_to_analyze = [str(i) for i in range(1, 23)] + ['X', 'Y']
