...
# Clean up population column - remove NaN values and convert to string
df = df.dropna(subset=['population'])  # Remove rows with NaN population
df['population'] = df['population'].astype(str)  # Ensure all are strings

# Filter out any remaining problematic entries
df = df[df['population'].str.len() > 0]  # Remove empty strings
df = df[~df['population'].isin(['nan', 'None', 'null'])]  # Remove string versions of null
...
population_colors = {
    ...
    "ASL": "rgba(255, 105, 180, 0.7)",  # Added missing ASL population
}
...
for pop in populations:
    if pop in ['REF', 'NA', '', 'nan']:  # Skip reference or empty populations
        continue
...
# Add chromosome boundaries - FIXED to handle X/Y chromosomes
...
# Filter populations for toggle view
...
for i, pop in enumerate(non_ref_populations):
    ...
    fig2.add_trace(
        go.Scatter(
            ...
            visible=(i == 0),  # Only first population visible by default
            hovertemplate=
            f'{pop}<br>' +
            'Chr: %{customdata[0]}<br>' +
            'Position: %{customdata[1]:,}<br>' +
